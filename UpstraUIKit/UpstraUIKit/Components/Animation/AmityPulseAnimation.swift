...
final class AmityPulseAnimation: CALayer {
    ...
    init(numberOfPulse: Float = Float.infinity, radius: CGFloat, postion: CGPoint){
        ...
        let backgroundQueue = DispatchQueue(label: "amityuikit.pulsee", qos: .default)
        backgroundQueue.async { [weak self] in
            guard let strongSelf = self else { return }
            strongSelf.setupAnimationGroup()
            DispatchQueue.main.async {
                strongSelf.add(strongSelf.animationGroup, forKey: "pulse")
           }
        }
    }
